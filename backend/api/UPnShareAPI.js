...
class UPnShareAPI {
    ...
    async listFiles(page = 1, retries = 3) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                console.error(`UPnShare listFiles page ${page} attempt ${attempt} error:`, error.message);
                if (attempt < retries) await sleep(1000 * attempt);
            }
        }
        console.error(`UPnShare listFiles page ${page}: all retries failed, returning []`);
    ...
    async listAllFiles() {
        if (_cache && Date.now() - _cacheTime < CACHE_TTL) {
            console.log(`  -> UPnShare: Cache hit (${_cache.length} files)`);
            return _cache;
        }
    ...
    async renameFile(fileId, newName) {
        try {
            ...
            return { success: true, platform: 'upnshare', message: 'File renamed successfully' };
        } catch (error) {
    ...
    async deleteFile(fileId) {
        try {
            ...
            console.log(`UPnShare deleteFile ${fileId}: HTTP ${res.status}`);
            return { success: true, platform: 'upnshare', fileId, message: 'File deleted successfully' };
        } catch (error) {
            const status = error.response?.status;
            const msg = error.response?.data?.message || error.message;
            console.error(`UPnShare deleteFile error for ${fileId}: HTTP ${status} — ${msg}`);
            return { success: false, platform: 'upnshare', fileId, error: msg };
        }
    }
}
